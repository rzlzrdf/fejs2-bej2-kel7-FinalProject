...
const Semua = () => {
  ...
  useEffect(() => {
    ...
    if (user && !product) {
      axios
        .get(
          "https://secondhandapp.herokuapp.com/api/product/all?size=20&page=1",
          {
            headers: {
              Authorization: user,
            },
  ...
  const changePage = (event) => {
    if (user) {
      axios
        .get(
          `https://secondhandapp.herokuapp.com/api/product/all?page=${event}&size=20`,
          {
            headers: {
              Authorization: user,
            },
  ...
  const changeCategory = (event) => {
    if (event === "all") {
      axios
        .get(`https://secondhandapp.herokuapp.com/api/product/all?size=15`)
        .then((res) => {
          console.log(res);
          setProducts(res.data.content);
        })
      ...
      axios
        .get(
          `https://secondhandapp.herokuapp.com/api/product/list?id=${event}&size=15`
        )
        .then((res) => {
          console.log(res);
          setProducts(res.data.content);
            ...
            <Col lg={10} className={style.all}>
              {loading ? (
                ...
                products.map((semua, index) => {
                  return <Cardss key={`Product-${index}`} product={semua} />;
                })
              )}
            </Col>
          )}
